...
function runImport(docs){
    ...
    return fetchPlantsRec(8700, lastKnownId);
}
...
function fetchPlants(from){
    return new Promise(function(resolve, reject){
        ...
        for ( var i = from; i < to; i++ ){
            util.log("\tfetching "+i+"\n");
            progress.push( rhsUtil.getPlantDetails( i ).then( handlePlant ).catch(console.err));
        }
...
function savePlant(plant){
    return new Promise(function(resolve, reject){
        ...
        plantsCollection.find({ "ids.rhs.id": plant.ids.rhs.id }, {rhs: 1})
          .then(function(docs){

            var md5 = util.md5( JSON.stringify(plant));
            if ( docs.length ){
